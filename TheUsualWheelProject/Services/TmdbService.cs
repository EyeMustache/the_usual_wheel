...
public class TmdbService : LoggingBase<TmdbService>
{
    ...
    //         Logger.LogInformation("DEBUG: Constructed movie details for TMDb ID {tmdbId}: {@movie}", tmdbId, constructedMovie.ToString());
    ...
    public async Task<TmdbMovie?> GetTmdbMovieDetailsAsync(int tmdbId)
    {
        Logger.LogInformation($"Fetching movie details from TMDb for ID {tmdbId}.");
        var tmdbMovie = await _tmdbClient.GetMovieAsync(tmdbId, MovieMethods.Credits);
        if (tmdbMovie == null)
        {
            Logger.LogWarning($"No movie found on TMDb for ID {tmdbId}.");
            return null;
        }

        TmdbMovie tmdbMovieDetails = new()
        {
            TmdbId = tmdbMovie.Id,
            Title = tmdbMovie.Title ?? "Unknown Title",
            Year = tmdbMovie.ReleaseDate?.Year ?? 0,
            Genre = tmdbMovie.Genres != null && tmdbMovie.Genres.Any()
                ? string.Join(", ", tmdbMovie.Genres.Select(g => g.Name))
                : "Unknown Genre",
            Director = tmdbMovie.Credits?.Crew?.FirstOrDefault(c => c.Job == "Director")?.Name ?? "Unknown Director",
            Cast = tmdbMovie.Credits?.Cast != null && tmdbMovie.Credits.Cast.Any()
                ? string.Join(", ", tmdbMovie.Credits.Cast.Take(5).Select(c => c.Name))
                : "Unknown Cast",
            DurationInMinutes = tmdbMovie.Runtime ?? 0,
            Synopsis = tmdbMovie.Overview ?? "Unknown Synopsis",
        ...
        Logger.LogInformation($"DEBUG: Constructed TMDb movie details for ID {tmdbId}: {tmdbMovieDetails}");
